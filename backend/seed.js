...
const seedDB = async () => {
  try {
    ...
    const owner1 = await User.create({ name: 'Rajesh Kumar', email: 'owner@demo.com', password: hashedPass, role: 'owner' });
    const owner2 = await User.create({ name: 'Sunita Sharma', email: 'owner2@demo.com', password: hashedPass, role: 'owner' });
    await User.create({ name: 'Demo Tenant', email: 'tenant@demo.com', password: hashedPass, role: 'tenant' });
    ...
    const seedData = [
      {
        ...
        owner: { name: owner1.name, phone: "+91 98765 43210", since: "2019", userId: owner1._id },
        rooms: [
          { type: "Single", price: 8500, available: 2 },
          { type: "Double Sharing", price: 6000, available: 1 },
          { type: "Triple Sharing", price: 4500, available: 3 },
      ...
      },
      {
        ...
        amenities: ["WiFi", "Meals", "Laundry", "Security", "CCTV"],
        description: "Safe and comfortable accommodation exclusively for working women and students.",
        owner: { name: owner2.name, phone: "+91 87654 32109", since: "2020", userId: owner2._id },
        rooms: [
          { type: "Single", price: 7000, available: 1 },
          { type: "Double Sharing", price: 5000, available: 3 },
        ]
    ...
    await PG.insertMany(seedData);
    console.log('6 PGs seeded ✅');
    console.log('\nDemo accounts:');
    console.log('  Owner  → owner@demo.com / password');
    console.log('  Tenant → tenant@demo.com / password');
