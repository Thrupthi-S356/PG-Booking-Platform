...
export default function PGCard({ pg }) {
  ...
  return (
    <div className="glass rounded-2xl overflow-hidden card-hover group border border-white/5 hover:border-brand-500/30">
      ...
      <div className="relative h-48 overflow-hidden bg-slate-800">
        ...
        {!pg.available && (
          <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
            <span className="text-white font-semibold text-sm bg-red-500/80 px-4 py-1.5 rounded-full">
              Fully Occupied
            </span>
          </div>
        )}
      ...
      {/* Content */}
      <div className="p-4">
        <h3 className="font-semibold text-white text-base leading-tight mb-2">{pg.title}</h3>
