...
// ─────────────────────────────────────────────────────────────────────────────
// ALL helper components OUTSIDE OwnerDashboard so React never remounts them
// on every keystroke (defining components inside another component = focus lost)
// ─────────────────────────────────────────────────────────────────────────────
...
const PGFormBody = ({ data, setData, onSubmit, loading: busy, onCancel, submitLabel }) => (
  <div className="space-y-4 max-h-[72vh] overflow-y-auto pr-1">
    ...
    {/* image upload */}
    <Field label="PG photo">
      <div className="border border-dashed border-white/20 rounded-xl p-4 hover:border-brand-500/40 transition-colors">
        <input type="file" accept="image/*" id="pg-img-upload" className="hidden"
          onChange={e => {
            const file = e.target.files[0];
            if (!file) return;
            const reader = new FileReader();
            reader.onloadend = () => setData(p => ({ ...p, images: reader.result }));
            reader.readAsDataURL(file);
          }} />
        <label htmlFor="pg-img-upload" className="flex flex-col items-center gap-2 cursor-pointer">
          {data.images && (data.images.startsWith('data:') || data.images.startsWith('http')) ? (
            <img src={data.images} alt="preview" className="w-full h-32 object-cover rounded-lg" />
          ) : (
            <>
      ...
      <input
        ...
        value={data.images && !data.images.startsWith('data:') ? data.images : ''}
        onChange={e => setData(p => ({ ...p, images: e.target.value }))}
        className={`${inp} mt-2`} />
    </Field>
...
// ─────────────────────────────────────────────────────────────────────────────
// blank state — also outside so it's never recreated
// ─────────────────────────────────────────────────────────────────────────────
const blankPG = {
  title: '', city: '', area: '', price: '',
  phone: '', type: 'coed', images: '', available: true,
  ...
  const handleAddPG = async () => {
    ...
    try {
      const created = await pgService.create({
        ...
        type:       newPG.type,
        available:  true,
        images:     [newPG.images || 'https://images.unsplash.com/photo-1555854877-bab0e564b8d5?w=800&q=80'],
        amenities:  ['WiFi'],
      ...
      showToast('Failed to add PG', 'error');
  ...
  const handleEditSave = async () => {
    ...
    try {
      const updated = await pgService.update(editPG._id, {
        ...
        images:    [editPG.images || 'https://images.unsplash.com/photo-1555854877-bab0e564b8d5?w=800&q=80'],
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          ...
                          <div className="flex items-center gap-0.5">
                            {[1,2,3,4,5].map(i => (
                              <Star key={i} size={11}
                                className={i <= Math.round(safeRating) ? 'fill-amber-400 text-amber-400' : 'text-slate-600'} />
                            ))}
