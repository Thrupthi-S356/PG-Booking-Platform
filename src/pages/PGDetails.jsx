...
import { MapPin, Wifi, Coffee, Wind, Shield, Car, WashingMachine, ChevronLeft, ChevronRight, Phone, CheckCircle, Dumbbell, Camera, Zap, Home } from 'lucide-react';
          ...
          <div className="relative rounded-2xl overflow-hidden bg-slate-800 h-80 md:h-96 group">
            ...
            {pg.images.length > 1 && (
              <>
                <button
                  onClick={prevImg}
                  className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 glass rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white/20"
                >
                  <ChevronLeft size={18} />
                </button>
                <button
                  onClick={nextImg}
                  className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 glass rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white/20"
                >
            ...
            <div className="absolute bottom-4 right-4 flex gap-2">
              {pg.images.map((img, i) => (
                <button
                  key={i}
                  onClick={() => setImgIdx(i)}
                  className={`w-12 h-9 rounded-lg overflow-hidden border-2 transition-all ${i === imgIdx ? 'border-brand-400' : 'border-white/20'}`}
                >
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <div className="flex items-center gap-2 text-slate-400">
                  ...
                  <span className="capitalize px-2 py-0.5 text-xs rounded-full bg-white/10 text-slate-300">
                    {pg.type}
                  </span>
                </div>
              </div>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 glass rounded-2xl border border-white/10 p-6 space-y-5">
            <div>
              ...
              <p className="text-3xl font-bold text-brand-400 font-display">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={e => setBookingForm(f => ({ ...f, roomType: e.target.value }))}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button
              ...
              onClick={handleBook}
              disabled={!bookingForm.roomType || !bookingForm.checkIn}
            >
              Send Request
            </Button>
          </div>
